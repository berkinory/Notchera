...
final class YouTubeMusicController: MediaControllerProtocol {
    ...
    init(configuration: YouTubeMusicConfiguration = .default) {
        ...
        httpClient = YouTubeMusicHTTPClient(baseURL: configuration.baseURL)
        authManager = YouTubeMusicAuthManager(httpClient: httpClient)
    ...
    func fetchShuffleState() async {
        await sendCommand(endpoint: "/shuffle", method: "GET", refresh: false)
    }

    func fetchRepeatMode() async {
        await sendCommand(endpoint: "/repeat-mode", method: "GET", refresh: false)
    }
    ...
    private func sendCommand(
        ...
        do {
            ...
            if endpoint == "/shuffle" {
                ...
                let nextMode: RepeatMode = switch playbackState.repeatMode {
                case .off: .all
                case .all: .one
                case .one: .off
                }
